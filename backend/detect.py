...
# }
CLASS_PROMPT_MAP = {
    "house, building, rooftop seen from above": "building",
    "road, highway, street viewed from above": "road",
    "car, truck, vehicle viewed from above": "vehicle",
    "tree, vegetation, grassland": "vegetation",
    "river, lake, water": "water body",
    "bare soil, empty land, open ground": "open ground",
}

PROMPT_CLASSES = list(CLASS_PROMPT_MAP.keys())
CLASSES = list(CLASS_PROMPT_MAP.values())
...
def get_avg_color(image: Image.Image, bbox):
    ...
    crop = np.asarray(image.crop((x1, y1, x2, y2)))
...
def remove_duplicates(detections, threshold=0.5):
    ...
    for det in detections:
        ...
        for existing in keep:
            ...
            if (
                det["class"] == existing["class"]
                and iou(det["bbox"], existing["bbox"]) > threshold
            ):
                duplicate = True
                break
...
# ----------------------------------------------------
@torch.no_grad()
def detect(image: Image.Image, conf_threshold=0.05):

    model = load_model()

    resized = resize_for_inference(image)

    img_np = np.asarray(resized.convert("RGB"))
    ...
    for idx, box in enumerate(r.boxes):
        ...
        prompt_name = PROMPT_CLASSES[cls_id]

        label = CLASS_PROMPT_MAP[prompt_name]
        ...
        #     }
        # )
        # Reject weak detections for this class
...
def draw_boxes(image: Image.Image, detections):
    ...
    for det in detections:
        ...
        colour = colours.get(det["class"], "white")
...
if __name__ == "__main__":
    ...
    detections, resized = detect(img)

    print(detections)

    out = draw_boxes(resized, detections)
